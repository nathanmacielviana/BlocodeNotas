"use strict"; //Elimina erros e otimiza
...
const deleteAllNote = document.querySelector(".delete_all");
...
function eliminarNota()
{
    ...
    {
        let resposta = confirm("Deseja excluir essa nota?");
        if(resposta)
        {
    ...
    saveNote.addEventListener("click", (e)=>{
        ...
        if(valorAnteriorTitle != valorAtualTitle || valorAnteriorText != valorAtualText){
            if(valorAtualTitle.length != 0 && valorAtualText.length != 0){
                let resposta = confirm("Deseja salvar essa nota?");
                if(resposta){
